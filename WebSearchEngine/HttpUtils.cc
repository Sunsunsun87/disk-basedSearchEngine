...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  char* absolutePath = (char*) malloc (ABSOLUTE * sizeof(char));
  if (realpath(test_file.c_str(), absolutePath) == NULL) {
    return false;
  }
  string abP(absolutePath);
  free(absolutePath);
  // see if root_dir preceeds test_file.

  string root_dir_append(root_dir);
  root_dir_append.append("/");
  if (abP.find(root_dir_append) != string::npos) {
    return true;
  }
