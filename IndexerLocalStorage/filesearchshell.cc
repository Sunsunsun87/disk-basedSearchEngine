...
static void print(vector<hw3::QueryProcessor::QueryResult> result);

// Getting input query words.
static std::vector<std::string> getQueryWords();
...
int main(int argc, char** argv) {
  ...
  // put the given arguments into a string list
  for (int i = 1; i < argc; i++) {
    index_list.push_back(string(argv[i]));
  }

  hw3::QueryProcessor queryprocessor(index_list, true);

  while (1) {
    // get the user input and store them into a vector
    std::vector<std::string> queryWords = getQueryWords();
    // Check if the loop terminated due to EOF
    if (std::cin.eof()) {
    ...
    // process the query
    result = queryprocessor.ProcessQuery(queryWords);
...
static void print(vector<hw3::QueryProcessor::QueryResult> result) {
  ...
  for (auto r : result) {
    std::cout << "  " << r.document_name << " (" << r.rank << ")" << endl;
  }
}

static std::vector<std::string> getQueryWords() {
  std::vector<std::string> queryWords;
  std::string word, user_input;
