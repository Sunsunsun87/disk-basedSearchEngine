...
for (int i = 0; i < header_.num_buckets; i++) {
    // STEP 4.
    // Seek to the next BucketRecord.  The "offset" member
    // variable stores the offset of this docid table within
    // the index file.
    IndexFileOffset_t pos = offset_ +
                     sizeof(BucketListHeader) + i * sizeof(BucketRecord);
    Verify333(fseek(file_, pos, SEEK_SET) == 0);
    // STEP 5.
