...
FileIndexReader::FileIndexReader(const string& file_name,
  ...
  // Verify333(fread(&fileHeader, sizeof(IndexFileHeader), 1, file_)
      // == sizeof(IndexFileHeader));
  // int readbytes = fread(&fileHeader, sizeof(IndexFileHeader), 1, file_);

  Verify333(read(fd, &fileHeader, sizeof(IndexFileHeader)) ==
        sizeof(IndexFileHeader));
  fileHeader.ToHostFormat();
  header_ = fileHeader;

  // STEP 3.
  // Verify that the magic number is correct.  Crash if not.
  Verify333(fileHeader.magic_number == 0xCAFEF00D);
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      // int bytes = fread(buf, sizeof(uint8_t), kBufSize, file_);
      int bytes = read(fd, buf, kBufSize);
