...
void MemIndex_AddPostingList(MemIndex* index, char* word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    //       mapping.
    //   (3) insert the new WordPostings into the inverted index (ie, into
    //       the "index" table).
    wpg = (WordPostings *) malloc(sizeof(WordPostings));
    wpg->word = word;  // direct usage of input word, no malloc and copy needed.
    wpg->postings = HashTable_Allocate(10);
    mi_kv.key = key;
    mi_kv.value = wpg;
  ...
  // Because we only insert a word in a doc one time.
  Verify333(!HashTable_Find(wpg->postings, doc_id, &postings_kv));

  // STEP 3.
  // Insert a new entry into the wpg->postings hash table.
  // The entry's key is this docID and the entry's value
  // is the "postings" (ie, word positions list) we were passed
  // as an argument.

  // Insert a KV into wpg->postings. KV: docID, postings.
  postings_kv.key = doc_id;
  postings_kv.value = postings;
  HashTable_Insert(wpg->postings, postings_kv, &unused);
}

LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  LinkedList* ret_list;
  HTKeyValue_t postings_kv, return_kv;
  WordPostings* wpg;
  HTKey_t key;
  int i;
  ...
  // HashTable_Find(index, FNV(query[0]), wp);
  // loop through wp->postings, each doc create a SearchResult, add to ret_list
  // HTIterator(wp->postings). HTI.Next, store in kv.
  key = FNVHash64((unsigned char*)query[0], strlen(query[0]));
  if (!HashTable_Find(index, key, &return_kv)) {
    // is there pointer type-casting issues? should I not use wpg ptr above?
    return NULL;
  }

  // looking for postings_kvs from index query result.
  wpg = return_kv.value;
  ret_list = LinkedList_Allocate();
  HTIterator * it;
  for (it = HTIterator_Allocate(wpg->postings);
     HTIterator_IsValid(it); HTIterator_Next(it)) {
    HTIterator_Get(it, &postings_kv);
    SearchResult * sr = (SearchResult *) malloc(sizeof(SearchResult));
    sr->doc_id = postings_kv.key;
    sr->rank = LinkedList_NumElements(postings_kv.value);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    wpg = (WordPostings*)return_kv.value;
    // Check docID..
    ...
    HTKeyValue_t query_kv;  // a postings HastTable
    SearchResult* sr;
    for (j = 0; j < num_docs; j++) {
      LLIterator_Get(ll_it, (LLPayload_t)&sr);  // get ret_list's current entry
      if (HashTable_Find(wpg->postings, sr->doc_id, &query_kv)) {
        // using docID key to find current ret_list entry
        // in query[i]'s postings table
        // we found it.
        // update this entry's rank
        sr->rank += LinkedList_NumElements(query_kv.value);
        LLIterator_Next(ll_it);
      } else {
        LLIterator_Remove(ll_it, (LLPayloadFreeFnPtr)free);
      }
    }
    // finished updating this rounds query result. Free iter.
