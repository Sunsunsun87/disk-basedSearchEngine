...
static void HandleDir(char* dir_path, DIR* d, DocTable** doc_table,
  ...
  for (i = 0; (dirent = readdir(d)) != NULL; i++) {
    ...
    // How do you compare strings in C?       strcmp. 0: same.
    if (!strcmp(dirent->d_name, ".") || !strcmp(dirent->d_name, "..")) {  // == '.'
    // next iteration i++, so i stays the same (initially at 0),
    // and entries[] will not store irrelevant entries
      i--;
      continue;
    }
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      //
      if (S_ISREG(st.st_mode)) { entries[i].is_dir = false; }
      //
      ...
      else if (S_ISDIR(st.st_mode)) { entries[i].is_dir = true; }
      //
      //
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  // file => WP Table
  char * file_content;
  file_content = ReadFileToString(file_path, &file_len);
  ...
  while (HTIterator_IsValid(it)) {
    ...
    // Now we have a wp table and a DocTable,
    // now to link these two using Mem..AddPostingList.
    ...
    // Extract wp from wp table using HTI_Remove;
    // Then add using MemIdx_Add...
    // Each one? Of course! We need to extract and inject the entire wp table.
    // Eventually there will be an empty prior wp table
    HTIterator_Remove(it, &kv);
    wp = kv.value;  // one word, and its occurences in this file
