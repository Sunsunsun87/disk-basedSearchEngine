...
//   Attempt to open the file for reading (see also "man 2 open").
  ...
  while (left_to_read > 0) {
    bytes = read(fd, buf + total, left_to_read);
    if (bytes < 0) {
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  char* end_ptr = cur_ptr + strlen(content);
  ...
  while (cur_ptr < end_ptr) {
    while (cur_ptr < end_ptr && !isalpha(*cur_ptr)) { cur_ptr++; }
    word_start = cur_ptr;
    while (cur_ptr < end_ptr && isalpha(*cur_ptr)) {
      *cur_ptr = tolower(*cur_ptr);
      cur_ptr++;
    }
    if (cur_ptr < end_ptr) {
      *cur_ptr = '\0';
      AddWordPosition(tab, word_start, word_start - start_ptr);
    }
  }
}
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // allocating positions list
    LinkedList * positions = LinkedList_Allocate();
    // pushing current pos value using hack
    LinkedList_Push(positions, (LLPayload_t)(int64_t) pos);
    wp = (WordPositions*) malloc(sizeof(WordPositions));
    wp->word = (char*) malloc(strlen(word) + 1);
    strcpy(wp->word, word);
    wp->positions = positions;  // defining wp.
    kv.key = FNVHash64((unsigned char*)word, strlen(word));
    kv.value = (HTValue_t) wp;  // defining kv with word and wp
    HTKeyValue_t oldkv;
    HashTable_Insert(tab, kv, &oldkv);  // inserting kv
  }
}
